...
std::string Base64::Encode(const unsigned char * str,int bytes) {
	...
	int encode_size = ((bytes - 1)/3 +1)*4+1;
	std::string s(encode_size, '\0');
	base64_encode((const char*)str, bytes, (char *)s.c_str(), (size_t *) &encode_size , BASE64_FORCE_NEON32);
	return s;
#else
    int num = 0,bin = 0;
...
std::string Base64::Decode(const char *str,int length) {
	...
	int decode_size = length*3/4 + 1;
	std::string s(decode_size, '\0');
	base64_decode((const char*)str, length, (char *)s.c_str(), (size_t *) &decode_size , BASE64_FORCE_NEON32);
	return s;
